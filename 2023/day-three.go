...
func partOneResults(list []string) int {
	...
	for i := 0; i < len(list); i++ {
		...
		for characterIndex, character := range items {
			...
			value := getNumberByIndex(items, characterIndex)
			valueLength := len(strconv.Itoa(value))

			if hasSymbolLeftOfNumber(items, characterIndex) || hasSymbolRightOfNumber(items, characterIndex, valueLength) || hasSymbolLineAbove(list, i, characterIndex, valueLength) || hasSymbolLineBelow(list, i, characterIndex, valueLength) {
				total += value
			}

			skipCount = valueLength - 1
		}
	}
...
func hasSymbolLeftOfNumber(items []string, index int) bool {
	return index != 0 && hasSymbol(items[index-1])
}

func hasSymbolRightOfNumber(items []string, index int, digits int) bool {
	return index+digits < len(items) && hasSymbol(items[index+digits])
}

func hasSymbolLineAbove(line []string, lineIndex int, index int, digits int) bool {
	return lineIndex != 0 && len(line[lineIndex-1]) > 0 && hasSymbolInRange(line[lineIndex-1], index, digits)
}

func hasSymbolLineBelow(line []string, lineIndex int, index int, digits int) bool {
	return lineIndex+1 <= len(line) && len(line[lineIndex+1]) > 0 && hasSymbolInRange(line[lineIndex+1], index, digits)
}

func hasSymbolInRange(value string, index int, amount int) bool {
	...
	for i := start; i < index+amount+1; i++ {
		if i < max && hasSymbol(items[i]) {
			return true
		}
	}
...
func hasSymbol(value string) bool {
	r, _ := regexp.Compile("(?i)[a-z0-9.]")
	return len(r.ReplaceAllString(value, "")) > 0
}
